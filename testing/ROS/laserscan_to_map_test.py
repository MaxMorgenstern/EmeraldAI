...
# Subscribers
range_sub = rospy.Subscriber("/dummy/scan", LaserScan, callback_range)

# Publishers
occ_pub = rospy.Publisher("/dummy/map", OccupancyGrid, queue_size = 10)
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		try:
			t = tflistener.getLatestCommonTime("/base_link", "/odom")
			position, quaternion = tflistener.lookupTransform("/odom", "/base_link", t)
		except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
			continue

		try:
			t = tflistener.getLatestCommonTime("/base_link", "/laser_frame")
			position_sonar, quaternion_sonar = tflistener.lookupTransform("/base_link", "/laser_frame", t)
		except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
			continue
